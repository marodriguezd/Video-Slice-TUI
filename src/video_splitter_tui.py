...
class VideoSplitterApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main"):
            with Vertical():
                yield Static("🎬 Video Splitter TUI", id="title")
                ...
                with Horizontal(id="time_inputs"):
                    with Vertical(classes="time_group"):
                        ...
                        self.duration_input = Input(placeholder="e.g., 30")
                        yield self.duration_input
                    
                    yield Button("Split", id="split_btn", variant="success")
...
def main():
    ...
    if len(sys.argv) > 1:
        arg = sys.argv[1]
    app = VideoSplitterApp(video_path_arg=arg)
    app.run()
